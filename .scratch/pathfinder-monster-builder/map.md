...
- A class progression has one primary class graft with an explicit level and optional distinct secondary/tertiary class grafts with their own levels. Only the primary controls the array, foundational statistics, skills, choices, and primary spellcasting; additional classes contribute level-appropriate automatic grants while their selectable option categories replace primary categories without increasing the slot count.
...
- [Welche lokale KI-Integration wird zuerst unterstützt?](issues/07-first-ai-adapter.md): Use an in-process Pi SDK adapter with Pi/provider defaults and an `openai-codex/gpt-5.6-luna` fallback. Require a hard-gated `catalog_list` call before read-only catalog/choice/validation tools and allow typed terminating `emit_proposal` only for the exact last valid candidate; Ollama is backlog, CR inference is visible `sum(class levels) - 1`, and user confirmation remains in `proposal.accept`.
- [Welche Tests beweisen die 1:1-Regeltreue?](issues/08-rule-fidelity-acceptance-tests.md): Test only the public `execute` interfaces with independent source expectations. Use Worg, Griffon, and pre-Reality-Check Medusa as strict golden fixtures; cover all tables, CR bands, grafts, options, spells, skills, damage, and exports. Multiclass cases use one primary class graft for foundational calculations and explicit level-tagged secondary grafts for automatic grants and replacement option categories; Reality Check and Free remain deferred.
