...
JSONL is only CLI framing; the operation contract is the same in-process and over the local browser transport. `proposal.validate`, `proposal.create`, `proposal.get`, and explicitly confirmed `proposal.accept` use the same deterministic Engine seam. The browser-only `proposal.generate` application operation uses one ephemeral Pi SDK `AgentSession`. It can inspect `draft_choice_requirements`, validate and repair up to three candidates through `proposal_validate`, and query only `catalog_list`, `catalog_search`, and `catalog_get`; `catalog_list` is a hard first gate, and terminating `emit_proposal` accepts only the exact last candidate validated as valid. It uses Pi's configured default/available model, then the documented `openai-codex/gpt-5.6-luna` fallback when no default resolves. Missing credentials, model, Node, or optional SDK packages produce visible AI errors without changing the Draft.
