...
class WebTransportTests(unittest.TestCase):
    ...
    def test_checked_in_ui_is_a_built_typescript_app_with_structured_editors(self):
        ...
        self.assertIn('Object.keys(objectValue(array?.presets))', npc_steps)
        self.assertNotIn('primaryClass === "npc-class.druid"', npc_steps)
        self.assertIn('selectionBudgets?.spells?.classId', npc_steps)
        self.assertIn('properties', npc_steps)
        self.assertIn('Weapon qualities', npc_steps)
        ...
        self.assertIn('draft.export', app)
        self.assertIn('derivedFrom', app)
