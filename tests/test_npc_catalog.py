...
class NpcCatalogTests(unittest.TestCase):
    ...
    def test_production_catalog_keeps_the_human_warrior_slice_resolved(self):
        ...
        self.assertTrue(all(race["catalogStatus"] == "gap" for race_id, race in catalog["races"].items() if race_id not in {"npc-race.human", "npc-race.goblin", "npc-race.halfling", "npc-race.elf", "npc-race.hobgoblin", "npc-race.gnome"}))
